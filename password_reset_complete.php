...
try {
    $stmt = $dbh->prepare(
        'SELECT pr.ringer_id FROM password_reset_token pr
    ...
    $ringerId = (int) $row['ringer_id'];
    $pwdHash = password_hash($pwd, PASSWORD_DEFAULT);
    if ($pwdHash === false || strlen($pwdHash) > 255) {
        reset_fail('Could not set password. Please try again.');
    }

    $dbh->prepare('UPDATE ringers SET password = :p WHERE id = :id')->execute([
        ':p' => $pwdHash,
        ':id' => $ringerId,
    ]);
